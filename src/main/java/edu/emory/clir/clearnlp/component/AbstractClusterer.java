...
abstract public class AbstractClusterer {
	...
	@Option(name="-type", usage="the type of vector space (default: "+AbstractClusterer.TYPE_STRING+")\n"+
			AbstractClusterer.TYPE_SPARSE+": sparse vector space\n"+
			AbstractClusterer.TYPE_STRING+": string vector space\n",
			required=false, metaVar="<byte>")
	protected byte i_vectorType = AbstractClusterer.TYPE_STRING;
	...
	{
		return (AbstractModel<I,F>)(vectorType == AbstractClusterer.TYPE_SPARSE ? new SparseModel(binary) : new StringModel(binary));
	}
	...
	public AbstractModel<?,?> loadModel(String modelFile, byte vectorType)
	{
		try
		{
			...
			AbstractModel<?,?> model = vectorType == AbstractClusterer.TYPE_SPARSE ? new SparseModel(in) : new StringModel(in);
			in.close();
			return model;
		}
